...
export function isE164PhoneNumber(value: string) {
  return /^\+[1-9]\d{7,14}$/.test(normalizePhoneToE164(value));
}

export function buildE164PhoneNumber(dialCode: string, nationalNumber: string) {
  const normalizedDialCode = normalizeDialCode(dialCode);
  const normalizedNationalNumber = normalizePhoneDigits(nationalNumber);
  if (!normalizedDialCode || !normalizedNationalNumber) {
    return '';
  }
  return `${normalizedDialCode}${normalizedNationalNumber}`;
}
...
export function splitE164PhoneNumber(value: string) {
  ...
  const matches = COUNTRY_DIAL_CODES
    .map((item) => ({ item, normalizedDialCode: normalizeDialCode(item.dialCode) }))
    .filter(({ normalizedDialCode }) => normalizedValue.startsWith(normalizedDialCode))
    .sort((left, right) => right.normalizedDialCode.length - left.normalizedDialCode.length);
  ...
  return {
    country: match.item,
    nationalNumber: normalizedValue.slice(match.normalizedDialCode.length),
  };
}
